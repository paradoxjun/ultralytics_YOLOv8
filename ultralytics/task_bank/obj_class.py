...
def filter_person_boxes(xyxy, conf, height_ratio=3.0, width_ratio=3.0, area_ratio=5.0, confidence=0.5):
    """
    返回有效的文本框索引。
...
class BankDetObject:
    ...
    返回目标的 [检测框, 置信度, 追踪ID]
    """
    def __init__(self, idx_frame, track_id, xyxy, confidence, min_confidence=0.5, max_history=5):
        ...
        self.history = deque([[idx_frame, xyxy, confidence]], maxlen=self.max_history)     # 保存的历史信息

    def update(self, idx_frame=None, xyxy=None, confidence=None):
        if not (idx_frame is None or confidence < self.min_confidence):    # 置信度高，历史信息作用小，当前结果加入历史信息
            self.add_to_history(idx_frame, xyxy, confidence)
            self.last_update_frame = idx_frame

        return self.xyxy, self.confidence, self.track_id                    # 返回最大置信度对应的检测结果

    def add_to_history(self, idx_frame, xyxy, confidence):      # 删除超过15帧的历史信息，相当于记录1秒内最有用的几帧
        ...
        if len(self.history) >= self.max_history:
            ...
            if confidence > self.history[min_confidence_index][2]:      # 如果最小置信度也比当前的大，当前帧的置信度就无效，不加入
                self.history[min_confidence_index] = [idx_frame, xyxy, confidence]
        else:
            self.history.append([idx_frame, xyxy, confidence])

        max_confidence_index = self.get_max_confidence_index()
        self.xyxy = self.history[max_confidence_index][1]
        self.confidence = self.history[max_confidence_index][2]

    def get_max_confidence_index(self):  # 返回最大置信度索引
        return max(range(len(self.history)), key=lambda i: self.history[i][2])

    def get_min_confidence_index(self):  # 返回最小置信度索引
        return min(range(len(self.history)), key=lambda i: self.history[i][2])
    ...
    def update(self, idx_frame, ycj_results, ren_result, ioa_threshold=0.5):
        ...
        for ycj in ycj_results:
            ...
            if track_id in self.track_id_map and self.track_id_map[track_id] in self.id_set:
                ...
                self.money_counters[real_id].update(idx_frame, xyxy, conf)
            else:
                ...
                for real_id, counter in self.money_counters.items():
                    if ioa(counter.xyxy, xyxy) > ioa_threshold:  # 假设IOA大于0.5认为是同一台验钞机
                        ...
                        counter.update(idx_frame, xyxy, conf)
                        found = True
                        break
                ...
                if not found:   # 新的验钞机
                    if track_id in self.track_id_map:
                        ...
                        real_id = self.nums
                    self.track_id_map[track_id] = real_id
                    self.money_counters[real_id] = BankDetObject(idx_frame, track_id, xyxy, conf)
        ...
        for real_id, counter in self.money_counters.items():
            valid_counters.append([*counter.xyxy, label, real_id, counter.confidence])
