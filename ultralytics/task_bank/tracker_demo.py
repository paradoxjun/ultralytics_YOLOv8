...
from ultralytics.task_bank.utils import get_config, resize_and_pad, get_bytetrack_input_by_label, draw_dashed_rectangle
from ultralytics.task_bank.process_ren import FilterPerson
from ultralytics.task_bank.process_ycj import MoneyDetectorTracker
from ultralytics.task_bank.process_kx import KXTracker
from ultralytics.task_bank.process_money import MoneyTracker
...
class VideoTracker:
    ...
    def plot_track(self, img, track_outputs):      # 在一帧上绘制检测结果（类别+置信度+追踪ID）
        # 加载中文字体
        font = ImageFont.truetype("/home/chenjun/code/ultralytics_YOLOv8/datasets/Aa般若楷书 (非商业使用).ttf", 20)
        ...
        for i, box in enumerate(outputs_person):
            ...
            label_text = f'人:{round(conf, 2)}'  # 左上角标签+置信度文字
            ...
            # 绘制中文文本
            draw.text((x1, y1 - 25), label_text, font=font, fill=(color[2], color[1], color[0], 255))
        ...
        for i, box in enumerate(outputs_ycj[0]):
            ...
            color = (64, 128, 256)    # 设置颜色
            cv2.rectangle(img, (x1, y1), (x2, y2), color, 2)    # 基本矩形检测框
            label_text = f'验钞机{real_id}: {round(confidence, 2)}'  # 左上角标签+置信度文字
            ...
            # 绘制中文文本
            draw.text((x1 - 20, y1 - 25), label_text, font=font, fill=(color[2], color[1], color[0], 255))
        ...
        for i, box in enumerate(outputs_ycj[1]):
            ...
            label_text = f'验钞机{real_id}: 被遮挡'  # 左上角标签+置信度文字
        ...
        for i, box in enumerate(outputs_money):
            ...
            if check_state == 1:
                label_text = f"未验钞: {round(confidence, 2)}"
            elif check_state == 2:
                label_text = f"验钞中: {round(confidence, 2)}"
            else:
                label_text = f"已验钞: {round(confidence, 2)}"
        ...
        for i, box in enumerate(outputs_kx[0]):
            ...
            if error > 0:
                ...
                color = (0, 192, 192)  # 设置颜色

            cv2.rectangle(img, (x1, y1), (x2, y2), color, 2)  # 基本矩形检测框
            label_text = f'置信度：:{round(confidence, 2)}'  # 左上角标签+置信度文字
            if error > 0:
                track_text = f"款箱状态: 操作异常，放入未验钞的钱"
            elif correct > 0:
                track_text = f"款箱状态: 操作正常，放入验钞的钱"
            else:
                track_text = f"款箱状态: 无操作，未放入钱"
            ...
            # 绘制中文文本
            draw.text((x1 + 100, y2 - 25), label_text, font=font, fill=(color[2], color[1], color[0], 255))
            draw.text((x1, y1 - 25), track_text, font=font, fill=(color[2], color[1], color[0], 255))
